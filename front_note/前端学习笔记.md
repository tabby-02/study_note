...
```vue
<li v-for="(item,index) in name1" >---------------------------------------name1为对象数组，index可选
      {{ item.text }}
</li>
data() {return {
      name1: [
...
```vue
<li v-for="name1 in f1">{{ name1 }}</li>------------------------------------------------name1未定义，只会迭代显示f1返回值
data() {
        return { numbers: [  ,  ,  ,  ] }
    },
 computed: {
        f1() {return this.numbers.filter(number => number % 2 === 0)}-----该函数返回用filter()选择的numbers中为偶数的数
    }
```
